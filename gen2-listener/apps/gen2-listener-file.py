...
miller            = 2           # Number of Miller sub-carriers your reader communicates to tags
rtcal		  = 72          # Reader-To-Tag calibration (in microsec) of your reader --> this is the value used by Buettner's reader
dec_rate          = 16    	# decimation factor at USRP
sw_dec            = 4		# software decimation
samples_per_pulse = 12		# samples per pulse of the tag signal. It's calculated according as (1/(2*up_link_freq*1000))/((dec_rate*sw_dec)/64e6)
'''
...
'''
samp_rate = 2e6 #samples so we can capture 2 MHz of signal
rx_gain = 20 #20 dbM??
us_per_sample = 9 #??
...
class my_top_block(gr.top_block):
	...
	null_sink = blocks.null_sink(gr.sizeof_float*1)
	...
	# Enable real-time scheduling	
        ...
        # Create flow-graph
        mfilt = blocks.file_sink(gr.sizeof_float, 'f_mfilt.out')
        #debug
        self.connect(rx, to_mag, matched_filter, reader_monitor_cmd_gate, cr, tag_monitor, null_sink)
        self.connect(reader_monitor_cmd_gate, mfilt)
...
def main():
    ...
    parser.add_option("-f", "--file", action="store", dest="file_input", default="rx_in.out", help="Provide an input file for offline processing")
    
    (options, args) = parser.parse_args()	
    samp_freq = 2e6
    
    # FILE SOURCE for offline tests (comment previous lines and uncomments this line)
    rx = blocks.file_source(gr.sizeof_gr_complex*1, options.file_input, False)
    ...
    print "Using a offline file: ", options.file_input
    ...
    matched_filter = filter.fir_filter_fff(sw_dec, taps)
